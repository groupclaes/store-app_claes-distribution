    ...
    return this._db.executeQuery<any>(async (db: SQLiteDBConnection) => {
      ...
      for (const itemnum of products) {
        const productResult = await db.query(
          `SELECT itemnum, products.id, products.${nameString} as name, packingUnits.${nameString} as unit,('${environment.pcm_url}/product-images/' || products.itemnum || '?s=thumb') as url ` +
          'FROM products ' +
